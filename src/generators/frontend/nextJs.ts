...
export const nextJsGenerator = {
  generateTest(spec: ParsedApiSpec): string {
    ...
    return `import { waitFor } from '@testing-library/react';

// Assuming we are testing a frontend API client for Next.js
...
describe('Next.js API Client Tests - ${endpoint}', () => {
  beforeEach(() => {
  ...
  it('Edge Case (empty string payload)', async () => {
    ...
    expect(global.fetch).toHaveBeenCalledWith('${endpoint}', expect.objectContaining({
      body: JSON.stringify(${emptyStringBody}),
    }));
  });
